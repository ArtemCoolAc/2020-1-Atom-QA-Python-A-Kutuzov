...
from ui.locators.basic_locators import email, password
import time
...
class BasePage(object):
    ...
    def authentication(self, login, password, timeout=5):
        signin_button = self.find(self.locators.signin)
        signin_button.click()
        login_input = self.find(self.locators.login)
        password_input = self.find(self.locators.password)
        login_input.send_keys(login)
        password_input.send_keys(password)
        confirm_button = self.find(self.locators.auth_button)
